...
export function planEntryLevels(
  ...
  if (config.exit.stopLoss.type === "liquidity") {
    ...
    /*
     * Скоплений против сделки нет — СДЕЛКИ НЕТ.
     *
     * Раньше здесь подставлялась цена входа, и стоп вырождался в ширину
     * «запаса»: 0.25–1 ATR против обычного пула 1.5–3, то есть в три-двенадцать
     * раз у́же. Замер показал, во что это обходилось: 11.8% сделок семейства
     * шли с таким стопом, их МЕДИАННЫЙ результат был ровно −1R (то есть больше
     * половины — гарантированный вынос), и вклад в матожидание составлял
     * −0.045R при всём измеренном крае +0.0245R. Дефект был БОЛЬШЕ предмета
     * исследования.
     *
     * Ошибка была смысловая, а не арифметическая: величина «запас ЗА уровень»
     * использовалась как «ширина стопа». Типы совпадают, единицы совпадают,
     * смысл — нет.
     *
     * Почему отказ, а не подстановка нормального ATR-стопа: правило называется
     * «стоп за ВСЕЙ ликвидностью против сделки». Нет ликвидности — у правила
     * нет определённого стопа, и подставлять любую величину значит доизобретать
     * правило, которого никто не объявлял. Выбирать же более удобное
     * изобретение ПОСЛЕ того, как увидел цифры, — подгонка.
     *
     * Прецедент внутри движка: цель этих же семейств уже так и работает —
     * «нет магнита впереди, сделки нет» (ветка takeProfit ниже).
     * Пре-регистрация: researcher/docs/degenerate-stop-preregistration.md.
     */
    if (!Number.isFinite(far)) return null;
    stopPrice = long ? far - buffer : far + buffer;
  } else {
