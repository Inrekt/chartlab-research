...
export interface LiquidityFeatures {
  ...
  atrAt: Float64Array;
  /**
   * Вес СНЕСЁННОГО на этом баре скопления (сильнейшего, если снесло несколько);
   * NaN — на баре ничего не снимали.
   *
   * Зачем это существует. В пре-регистрациях ДВУХ семейств заявлена одна и та
   * же проверка механизма: «край обязан быть СИЛЬНЕЕ там, где кластер стопов
   * был крупнее». Выполнить её было НЕЛЬЗЯ: снятие происходит до чтения
   * признаков (иначе получилось бы заглядывание вперёд), и уровень, который
   * только что снесли, из фич уже исчез. То есть доза — величина, ради которой
   * всё семейство и придумано, — была ненаблюдаема.
   *
   * Дефект наблюдаемости хуже обычного бага: он не даёт неверный ответ, он не
   * даёт НИКАКОГО, а заявленная в документе проверка выглядит выполненной.
   * Здесь она становится измеримой.
   *
   * Ноль заглядывания вперёд: пишется ровно то, что уже произошло на этом баре.
   */
  sweptAboveWeight: Float64Array;
  sweptBelowWeight: Float64Array;
  /** Цена снесённого скопления — чтобы отличать снос вплотную от сноса с запасом. */
  sweptAbovePrice: Float64Array;
  sweptBelowPrice: Float64Array;
}
