...
async function main(): Promise<void> {
  ...
  try {
    ...
    console.error(
      "\n⚠️ «Мягче в N раз» читать ОСТОРОЖНО. Номинальный уровень относится к\n" +
        "НУЛЕВОЙ популяции, а до поздних ворот доходят только кандидаты, уже\n" +
        "отобранные предыдущими воротами за прибыльность. Высокая доля\n" +
        "прохождения у них — ожидаемое следствие отбора, а не доказательство\n" +
        "мягкости. Проверять калибровку надо НЕОТОБРАННЫМИ кандидатами:\n" +
        "npx tsx researcher/oot-bench.ts --window in",
    );

    const sym = nullSymmetry(db);
    console.error("\n── проверка симметрии нуль-модели ──");
    console.error(
    ...
    if (sym.sampled > 100 && sym.negativeShare < 0.1) {
      console.error(
        "\nОтрицательных нет. Само по себе это НЕ означает сломанный базлайн:\n" +
          "выборка состоит из уже отобранных кандидатов, и положительный сдвиг у\n" +
          "них ожидаем. Замерено контрольным опытом (oot-bench --window in):\n" +
          "неотобранные кандидаты дают ~90% отрицательных t, то есть машинерия\n" +
          "исправна. Тревогу поднимать, только если контроль ТОЖЕ даст ~0%.",
      );
    }
