...
async function main(): Promise<void> {
  ...
  if (!full && !only) {
    ...
    // локально и в облаке.
    // ОБЪЕДИНЕНИЕ, а не «корпус, если он есть». Разница неочевидна, но
    // поймана на живом прогоне: отменённая сборка оставила в кэше 123 символа
    // из 162 объявленных, и дозапись «только по корпусу» законсервировала бы
    // недобор навсегда — корпус молча остался бы неполным, а объявленная
    // вселенная существовала бы только на бумаге. Объединение сводит корпус к
    // списку из git и всё так же не пускает в него все 832 символа архива.
    const known = new Set(scanCorpus(outDir).map((c) => c.symbol));
    const wanted = new Set([...known, ...committedUniverse()]);
    if (wanted.size > 0) {
