...
async function main(): Promise<void> {
  ...
  if (!full && !only) {
    ...
    // локально и в облаке.
    // Вселенная ОБЪЯВЛЯЕТСЯ в git, а не обнаруживается на диске.
    //
    // Пройдено два неверных варианта, и оба поймал живой прогон. «Только то,
    // что уже в корпусе» консервирует недобор: отменённая сборка оставила 123
    // символа из 162, и они такими бы и остались. «Объединение корпуса со
    // списком» — хуже: та же отменённая сборка натащила 97 символов из общего
    // листинга архива, объединение приняло их как свои, и вселенная поиска
    // выросла до 259 МОЛЧА. А состав вселенной определяет, что именно
    // измеряется, и меняться он должен видимым решением, а не остатками
    // прерванной закачки.
    //
    // Лишние файлы в каталоге при этом не удаляются: чужого мы не трогаем.
