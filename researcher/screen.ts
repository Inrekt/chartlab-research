...
/**
 * Пространству нечего предложить: все комбинации финансируемых семейств уже
 * испробованы.
 *
 * ОТДЕЛЬНЫЙ тип, а не обычная ошибка, потому что это принципиально разные
 * состояния: «машина сломалась» требует починки, «машине нечего делать» —
 * новой пре-регистрации от человека. Раньше оба падали одинаково, и ночь
 * уходила красной каждые сутки подряд. Красное каждый день перестаёт что-либо
 * значить: именно так и пропускают настоящую поломку среди привычного шума.
 */
export class SpaceExhaustedError extends Error {
