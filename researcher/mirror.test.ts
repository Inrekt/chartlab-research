...
describe("алиасы фьючерсных имён", () => {
  test("мем-коины есть в объявленной вселенной под СПОТОВЫМИ именами", () => {
    // На фьючерсах они торгуются пачками по 1000 (SHIBUSDT → 1000SHIBUSDT),
    // а файл корпуса называется спотовым именем — им пользуются фандинг,
    // метрики и весь остальной код. Алиас применяется при ЗАПРОСЕ, и забыть
    // его для нового источника значит молча потерять пять ликвидных монет:
    // именно так архивная сборка дала 157 символов вместо 162.
    const syms = committedUniverse();
    for (const meme of ["SHIBUSDT", "PEPEUSDT", "BONKUSDT", "FLOKIUSDT", "LUNCUSDT"]) {
      expect(syms).toContain(meme);
      expect(syms).not.toContain(`1000${meme}`);
    }
  });
});

describe("вселенная не теряется по дороге к источнику", () => {
  test("объявленные имена СПОТОВЫЕ, а архив знает фьючерсные — фильтровать нельзя", () => {
    // Ровно эта пара и дала 157 вместо 162 дважды подряд. Список объявлен
    // спотовыми именами (SHIBUSDT), архив отдаёт фьючерсные (1000SHIBUSDT),
    // алиас применяется ПОЗЖЕ — при запросе. Любое пересечение списка с
    // листингом источника отменяет собственную таблицу переименований.
    //
    // Тест держится за инвариант, а не за число: если в списке появится хоть
    // одно имя, которого нет в архиве под тем же написанием, пересечение
    // снова начнёт молча резать вселенную.
    const declared = committedUniverse();
    const aliased = ["SHIBUSDT", "PEPEUSDT", "BONKUSDT", "FLOKIUSDT", "LUNCUSDT"];
    const present = aliased.filter((s) => declared.includes(s));
    expect(present).toEqual(aliased);
    // Проверяем именно то свойство, из-за которого фильтр был опасен.
    expect(present.every((s) => !declared.includes(`1000${s}`))).toBe(true);
  });
});
